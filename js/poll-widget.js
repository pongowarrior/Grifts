...
const POLL_KEY = 'grifts-poll-data'; // Key to store poll data
...
function updateResultsUI() {
    ...
    if (total === 0) {
        ...
        resultsContainer.innerHTML = '<div style="text-align: center; padding: 1rem; color: var(--text-secondary);">No votes yet. Be the first!</div>';
        return;
    }
    ...
    resultsContainer.innerHTML = `
        <div id="result-fill-a" style="width: ${percentA}%; background-color: var(--accent-blue); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; height: 30px; transition: width 0.5s ease-in-out;">
            ${percentA > 10 ? `${percentA}%` : ''}
        </div>
        <div id="result-fill-b" style="width: ${percentB}%; background-color: var(--accent-green); color: var(--bg-dark); display: flex; align-items: center; justify-content: center; font-weight: bold; height: 30px; transition: width 0.5s ease-in-out;">
...
function handleVote(option) {
    if (loadFromMemory(VOTE_KEY)) {
        showAlert('You have already voted on this poll (session only)!', 'error');
        updateResultsUI();
        return;
    }
    ...
    saveToMemory(POLL_KEY, currentPoll);
 ...
 */
function getPollURL() {
    // Encode the question and options in URL
...
function loadPollFromURL() {
    ...
    if (q && a && b) {
        ...
        // Try to load existing votes from memory for this poll
        const savedPoll = loadFromMemory(POLL_KEY);
        if (savedPoll && savedPoll.q === currentPoll.q && savedPoll.a === currentPoll.a && savedPoll.b === currentPoll.b) {
            currentPoll.votes = savedPoll.votes;
        }
    }
}
...
function resetPoll() {
    ...
    showAlert('New poll created! Note: Votes only persist in your browser session.', 'info');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    shareButton.addEventListener('click', () => {
        ...
        showAlert('Poll link copied! Note: Votes are session-only and not shared across users.', 'info');
    });
});